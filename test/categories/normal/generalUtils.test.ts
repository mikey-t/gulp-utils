...
describe('simpleSpawnAsync', () => {
  const throwingScript = path.join(fixturesDir, 'nodeScriptThatThrows.js')

  it('does not throw when throwOnNonZero is false', async () => {
    const result = await simpleSpawnAsync('node', [throwingScript], { throwOnNonZero: false })
    console.log(result)
    assert.strictEqual(result.code, 1)
  })

  it('throws when throwOnNonZero is true', async () => {
    await assert.rejects(
      simpleSpawnAsync('node', [throwingScript], { throwOnNonZero: true }),
      err => assertErrorMessageIncludes(err, 'process failed with code 1')
    )
  })
})
